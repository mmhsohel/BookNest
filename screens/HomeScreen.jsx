...
const CardItem = memo(({ item, navigation }) => (
  <TouchableOpacity
    style={styles.card}
    onPress={() => navigation.navigate(item.screen, {bookData: item.itemData})}
  >
    <FastImage
...
if (evenCards.length % 2 !== 0) {
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.carouselWrapper, { height: IMAGE_HEIGHT }]}>
        <Carousel
          width={width * 0.9}
          height={IMAGE_HEIGHT}
          ...
          renderItem={({ item }) => (
            <FastImage
              ...
              style={[
                ...
                { width: width * 0.9, height: IMAGE_HEIGHT * 0.8 },
              ]}
              cacheKey={`carousel-${item.id}`}
              resizeMode="cover"
            />
          )}
        />
